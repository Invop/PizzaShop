...
using Polly.Retry;
namespace PizzaShop.EventBusRabbitMQ;
...
{
    private const string ExchangeName = "pizza_shop_event_bus";
    ...
    public Task PublishAsync(IntegrationEvent @event)
    {
        ...
        using IModel channel = _rabbitMQConnection?.CreateModel() ?? throw new InvalidOperationException("RabbitMQ connection is not open");
    ...
    private async Task OnMessageReceived(object sender, BasicDeliverEventArgs eventArgs)
    {
        ...
        {
            if (props.Headers.TryGetValue(key, out object? value) && value is byte[] bytes)
            {
        ...
        PropagationContext parentContext = _propagator.Extract(default, eventArgs.BasicProperties, ExtractTraceContextFromBasicProperties);
        ...
        using Activity? activity = _activitySource.StartActivity(activityName, ActivityKind.Client, parentContext.ActivityContext);
